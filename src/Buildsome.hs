...
executionLogBuildInputs bte@BuildTargetEnv{..} entity TargetDesc{..} Db.ExecutionLog {..} = do
    ...
    -- required:
    speculativeSlaves <- concat <$> traverse mkInputSlaves (M.toList elInputsDescs)
...
buildManyWithParReleased mkReason bte@BuildTargetEnv{..} entity slaveRequests =
    waitForSlavesWithParReleased bte entity =<< fmap concat (traverse mkSlave slaveRequests)
    where
        mkSlave req =
    ...
    where
        ...
        commonAccessHandler accessDoc rawInputs rawOutputs
            ...
            = do
                ...
                traverse_ (recordInput inputsRef reason) $
...
deleteOldTargetOutputs BuildTargetEnv{..} target = do
    ...
    traverse_ (removeOldOutput btePrinter bteBuildsome registeredOutputs) $
        targetOutputs target
...
clean printer buildsome = do
    ...
    Clean.Result _totalSize totalSpace count <-
        mconcat <$> traverse Clean.output (S.toList (outputs <> leaked))
